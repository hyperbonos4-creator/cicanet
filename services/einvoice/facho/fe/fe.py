...
# NAMESPACES según Anexo Técnico v1.9 - Resolución 165/2023
# Solo los namespaces NECESARIOS para evitar confusión del validador DIAN
NAMESPACES = {
    # Namespace Invoice (se usará prefijo 'fe' internamente, luego se elimina en tostring())
    'fe': 'urn:oasis:names:specification:ubl:schema:xsd:Invoice-2',
    # Namespace CreditNote (para Notas de Ajuste al Documento Soporte)
    'cn': 'urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2',
    # Namespaces UBL 2.1 estándar
    'cac': 'urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2',
    'cbc': 'urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2',
    'ext': 'urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2',
    # Namespace DIAN Colombia
    'sts': 'dian:gov:co:facturaelectronica:Structures-2-1',
    # Namespaces para firma digital XAdES
    'ds': 'http://www.w3.org/2000/09/xmldsig#',
    'xades': 'http://uri.etsi.org/01903/v1.3.2#',
    'xades141': 'http://uri.etsi.org/01903/v1.4.1#',
    # XMLSchema-instance para validación
...
class DianXMLExtensionCUDFE(FachoXMLExtension):
    ...
    def issue_time(self, datetime_):
        # DIAN Anexo Técnico v1.9 sec 11.2.1: HorFac DEBE incluir GMT (timezone) para cálculo CUFE
        # Ejemplo oficial: "HorFac: 10:53:10-05:00"
        return datetime_.strftime('%H:%M:%S-05:00')

    def issue_time_with_tz(self, datetime_):
        # Mismo formato que issue_time - mantener por compatibilidad
        return datetime_.strftime('%H:%M:%S-05:00')
...
class DianXMLExtensionCUDS(DianXMLExtensionCUDFE):
    ...
    Según Anexo Técnico v1.1 - Sección 14.1.1.1
    
    Fórmula: SHA-384(NumDS + FecDS + HorDS + ValDS + CodImp + ValImp + ValTot + NumSNO + NITABS + Software-PIN + TipoAmbiente)
    
    Diferencias con CUFE:
    - NumSNO: NIT del Vendedor (SNO - Sujeto No Obligado) sin DV
    - NITABS: NIT del Adquirente (ABS) sin DV
    ...
    def formatVars(self):
        """
        Formato según Anexo Técnico v1.1 página 214:
        NumDS + FecDS + HorDS + ValDS + CodImp + ValImp + ValTot + NumSNO + NITABS + Software-PIN + TipoAmbiente
        
        Nota: En Documento Soporte:
        - Supplier = Vendedor SNO (Sujeto No Obligado)
        - Customer = Adquirente ABS (quien genera el documento)
        """
        build_vars = self.buildVars()
        CodImpuesto1 = build_vars['CodImpuesto1']  # Siempre '01' para IVA
        
        # En DS: Supplier=SNO (vendedor), Customer=ABS (adquirente/nosotros)
        # NumSNO = NIT del Supplier (vendedor SNO) sin DV
        # NITABS = NIT del Customer (adquirente/nosotros) sin DV
        
        return [
            '%s' % build_vars['NumFac'],  # NumDS
            '%s' % build_vars['FecFac'],  # FecDS
            '%s' % build_vars['HoraFac'],  # HorDS
            form.Amount(build_vars['ValorBruto']).truncate_as_string(2),  # ValDS
            CodImpuesto1,  # CodImp (siempre '01')
            build_vars['ValorImpuestoPara'].get(CodImpuesto1, form.Amount(0.0)).truncate_as_string(2),  # ValImp
            form.Amount(build_vars['ValorTotalPagar']).truncate_as_string(2),  # ValTot
            '%s' % build_vars['NitOFE'],  # NumSNO (Supplier = Vendedor SNO)
            '%s' % build_vars['NumAdq'],  # NITABS (Customer = Adquirente)
            '%s' % build_vars['Software-PIN'],  # Software-PIN
            '%d' % build_vars['TipoAmb'],  # TipoAmbiente
        ]
...
class DianXMLExtensionSoftwareProvider(FachoXMLExtension):
    ...
    def build(self, fexml):
        ...
        #DIAN Anexo T5.1: FAB22 - schemeID debe ser el DV (dígito de verificación)
        #DIAN Anexo T5.1: FAB23 - schemeName debe ser '31' (tipo documento NIT)
        provider_id_attrs.update({'schemeID': self.dv})
        provider_id_attrs.update({'schemeName': '31'})
        #El valor del elemento debe ser solo el NIT SIN el DV
...
class DianXMLExtensionSigner:
    ...
    def sign_xml_element(self, xml):
        ...
        # Usar cryptography directamente en lugar de OpenSSL.crypto.load_pkcs12
        ...
        if additional_certificates:
            ctx.x509_ca = additional_certificates
...
class DianXMLExtensionAuthorizationProvider(FachoXMLExtension):
    ...
    def build(self, fexml):
        # Anexo T5.1: FAB34 - schemeID debe ser el DV de DIAN = "4"
        # FAB35 - schemeName debe ser "31" (tipo de documento NIT)
        # FAB31 - valor debe ser el NIT de DIAN sin DV = "800197268"
