...
class DIANInvoiceXML(fe.FeXML):
    ...
    def __init__(self, invoice, tag_document = 'Invoice'):
        # Según UBL 2.1, el namespace del elemento Invoice debe ser el oficial de OASIS UBL
        # No el namespace de la DIAN que es para las extensiones
        super().__init__(tag_document, 'urn:oasis:names:specification:ubl:schema:xsd:Invoice-2')
    ...
    def set_supplier(fexml, invoice):
        ...
        #DIAN 1.9-2023: FAJ24 - schemeID debe ser el DV, schemeName debe ser el tipo de documento (31 para NIT)
        ...
        #DIAN 1.9-2023: Telephone es mandatorio condicional (Anexo v1.9)
        if invoice.invoice_supplier.phone:
            fexml.set_element('./cac:AccountingSupplierParty/cac:Party/cac:Contact/cbc:Telephone',
                              invoice.invoice_supplier.phone)
    ...
    def set_customer(fexml, invoice):
        ...
        #DIAN 1.9-2023: FAK25 - Anexo Técnico v1.9
        #DIAN 1.9-2023: CAK25
        # Si DV está vacío, usar type_fiscal (ej: 13=Cédula, 31=NIT)
        ...
        customer_company_id_attrs.update({'schemeID': scheme_id_customer,
        ...
        for invoice_line in invoice.invoice_lines:
            for subtotal in invoice_line.tax.subtotals:
                if subtotal.scheme is not None:
                    ...
                    # FAS01b: Formatear Percent consistentemente con 2 decimales igual que en líneas
                    percent_for[subtotal.scheme.code] = '%0.2f' % round(subtotal.percent, 2)

                total_tax_amount += subtotal.tax_amount

        # COMENTADO: Este TaxTotal sin TaxSubtotal no cumple con FAS01-FAS04 del Anexo T5.1
        # Según el Anexo, TaxTotal DEBE contener TaxSubtotal (FAS04: ocurrencia 1..N)
    ...
    def set_invoice_line_tax(fexml, line, invoice_line):
        ...
        for subtotal in invoice_line.tax.subtotals:
            ...
            # TaxScheme es OBLIGATORIO según Anexo T5.1 FAS15 (1..1)
            if subtotal.scheme is not None:
                #DIAN 1.7.-2020: FAX15
                ...
                # Si no hay scheme, usar valores por defecto para IVA
                line.set_element('./cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID', '01')
                line.set_element('./cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:Name', 'IVA')
    ...
    def set_invoice_lines(fexml, invoice):
        ...
        for index, invoice_line in enumerate(invoice.invoice_lines):
            ...
            line.set_element('./cbc:%sQuantity' % (fexml.tag_document_concilied()), invoice_line.quantity, unitCode=invoice_line.quantity.code)
            fexml.set_element_amount_for(line,
            ...
            # DIAN 1.9-2023: Anexo Técnico v1.9 - No incluir atributos vacíos
            std_item_attrs = {}
            ...
            # Si no hay atributos válidos, usar valores por defecto DIAN
            if not std_item_attrs:
                std_item_attrs = {
                    'schemeAgencyID': '195',
                    'schemeName': '31',  # Estándar DIAN
                    'schemeID': '999'  # Estándar de adopción del contribuyente
                }
    ...
    def post_attach_invoice(fexml, invoice):
        #DIAN 1.9-2023: FAD03 - ProfileID según Anexo Técnico v1.9
        # Debe contener el literal completo para factura de venta
    ...
    def attach_invoice(fexml, invoice):
        ...
        #DIAN 1.7.-2020: FAD09 - IssueDate DEBE IR ANTES de DocumentCurrencyCode según UBL 2.1
        fexml.set_element('./cbc:IssueDate', invoice.invoice_issue.strftime('%Y-%m-%d'))
        #DIAN 1.9-2023: FAD10 - IssueTime DEBE incluir timezone -05:00 (zona horaria oficial de Colombia)
        fexml.set_element('./cbc:IssueTime', invoice.invoice_issue.strftime('%H:%M:%S-05:00'))
        #DIAN 1.9-2023: FAD12 - InvoiceTypeCode sin atributos según ejemplos oficiales DIAN v1.9
        fexml.set_element('./cbc:%sTypeCode' % (fexml.tag_document()),
                        invoice.invoice_type_code)
        #DIAN 1.7.-2020: FAD15 - DocumentCurrencyCode DESPUÉS de InvoiceTypeCode según Anexo T5.1
