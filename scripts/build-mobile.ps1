...
# --- Token (100% automatico: parametro > env > archivo local gitignored) -----
# Orden de busqueda, sin pedir nada por teclado:
#   1) -Token
#   2) $env:GH_TOKEN / $env:GITHUB_TOKEN
#   3) scripts/.gh-token  (archivo local NUNCA versionado; ver .gitignore)
if (-not $Token) { $Token = $env:GH_TOKEN }
if (-not $Token) { $Token = $env:GITHUB_TOKEN }
if (-not $Token) {
  $tokenFile = Join-Path $repoRoot "scripts/.gh-token"
  if (Test-Path $tokenFile) {
    $Token = ((Get-Content -Raw -Path $tokenFile) -replace '\s', '')
    if ($Token) { Info "Token leido de scripts/.gh-token (no se versiona)." }
  }
}
if (-not $Token) {
  Die "No hay token. Crea 'scripts/.gh-token' con tu PAT (permisos Contents + Actions), o pasa -Token, o define `$env:GH_TOKEN."
}
...
# --- API URL (auto-detecta el tunel ngrok si no se paso -ApiUrl) -------------
# Lee el inspector local de ngrok (127.0.0.1:4040) y usa el primer tunel https.
# Asi el build queda 100% automatico sin tener que pegar la URL a mano.
if (-not $ApiUrl) {
  try {
    $tun = Invoke-RestMethod -Uri "http://127.0.0.1:4040/api/tunnels" -TimeoutSec 3
    $pub = ($tun.tunnels | Where-Object { $_.public_url -like "https://*" } | Select-Object -First 1).public_url
    if ($pub) {
      $ApiUrl = "$($pub.TrimEnd('/'))/api"
      Info "ngrok detectado automaticamente."
    }
  } catch {
    Warn "No detecte ngrok en 127.0.0.1:4040 (arranca el tunel con scripts/tunnel.ps1)."
  }
}
